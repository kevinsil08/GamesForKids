<?php
	...
	function selectLastAdmin($cnn,$password,$email){
	    ...
	    $sql = "SELECT * FROM administrator WHERE adm_password='$md5pass' AND adm_email='$email'";
	    do
		if($result=mysqli_store_result($cnn)){
			mysqli_free_result($result);
	    } while(mysqli_more_results($cnn) && mysqli_next_result($cnn));
	...
	function loginAdmin($cnn, $email, $password){
		$md5pass = md5($password);
		$sql = "SELECT * FROM `administrator` WHERE adm_email='$email' AND adm_password='$md5pass'";
...
//Profesor
	...
	function loginTch($cnn, $email, $password){
		$md5pass = md5($password);
		$sql = "SELECT * FROM `teacher` WHERE tch_email='$email' AND tch_password='$md5pass'";
		$rs = mysqli_query($cnn,$sql);
		$fila = mysqli_fetch_assoc($rs);
		$nombreProfe=$email;
		return $fila;
	...
	function selectLastTch($cnn,$password,$email){
		$md5pass = md5($password);
	    $sql = "SELECT * FROM teacher WHERE tch_password='$md5pass' AND tch_email='$email'";
	    do
		if($result=mysqli_store_result($cnn)){
			mysqli_free_result($result);
	    } while(mysqli_more_results($cnn) && mysqli_next_result($cnn));

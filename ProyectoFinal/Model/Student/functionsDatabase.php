...
function insertStudent($cnn, $name, $last_name, $id_teacher){

    $sql = "CALL `pr_insert_student`('$name', '$last_name', $id_teacher);";
    do
	if($result=mysqli_store_result($cnn)){
		mysqli_free_result($result);
    } while(mysqli_more_results($cnn) && mysqli_next_result($cnn));
...
function updateStudent($cnn, $id_student, $name, $last_name){

    $sql = "CALL `pr_update_student`($id_student, '$name', '$last_name');";
    do
	if($result=mysqli_store_result($cnn)){
		mysqli_free_result($result);
    } while(mysqli_more_results($cnn) && mysqli_next_result($cnn));
